...
__weak int16_t PI_Controller( PID_Handle_t * pHandle, int32_t wProcessVarError )
{
    ...
    {
        wIntegral_Term = pHandle->hKiGain * wProcessVarError;//误差增益
        wIntegral_sum_temp = pHandle->wIntegralTerm + wIntegral_Term;//临时积分和

        if ( wIntegral_sum_temp < 0 )//临时积分和小于0
        {
            if ( pHandle->wIntegralTerm > 0 )//上一次积分大于0
            {
                if ( wIntegral_Term > 0 )//误差增益大于0
                {
                    wIntegral_sum_temp = INT32_MAX;//临时积分和最大
                }
            }
        }
        else
        {
            if ( pHandle->wIntegralTerm < 0 )//积分和小于0
            {
                if ( wIntegral_Term < 0 )//误差增益小于0
                {
                    wIntegral_sum_temp = -INT32_MAX;//临时积分和最小
                }
            }
        }
        //积分限制
...
#else
    /* 警告：以下说明不符合MISRA，用户应验证该Cortex-M3汇编指令ASR（算术右移）
             由编译器用来执行转换（而不是LSR）逻辑右移）*/
    wOutput_32 = ( wProportional_Term >> pHandle->hKpDivisorPOW2 ) + ( pHandle->wIntegralTerm >> pHandle->hKiDivisorPOW2 );
#endif

    if ( wOutput_32 > hUpperOutputLimit )//输出大于上限
    {
        wDischarge = hUpperOutputLimit - wOutput_32;//积分溢出时减少积分
        wOutput_32 = hUpperOutputLimit;//输出上限
    }
    else if ( wOutput_32 < hLowerOutputLimit )//输出低于下限
    {

        wDischarge = hLowerOutputLimit - wOutput_32;//积分下溢时增加积分
        wOutput_32 = hLowerOutputLimit;//输出下限
    }
    else { /* Nothing to do here */ }

    pHandle->wIntegralTerm += wDischarge;//累计积分

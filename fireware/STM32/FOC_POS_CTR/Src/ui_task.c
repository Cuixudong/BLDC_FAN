...
extern volatile int32_t tar_pos;
extern volatile int32_t g_encode_pos;
extern volatile int16_t g_encode_over;
...
//        pMCI[NBR_OF_MOTORS-1]->pFOCVars->bDriveInput = INTERNAL;
//        STC_SetControlMode( pMCI[NBR_OF_MOTORS-1]->pSTC, STC_TORQUE_MODE );
//        MCI_ExecTorqueRamp( pMCI[NBR_OF_MOTORS-1], 0, 0 );
...
void UI_HandlerStartPosInit(void)
{
    qd_t cur;
    cur.d = 0;
    cur.q = 0;
    if (MC_GetSTMStateMotor1() == IDLE)
    {
        tar_pos = (LL_TIM_GetCounter(ENCODER_M1.TIMx) + g_encode_over * 4000);
